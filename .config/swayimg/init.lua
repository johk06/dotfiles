...
color("gallery", "selected_color", 0xff8fbcbb)
color("gallery", "unselected_color", 0xff2e3440)
color("gallery", "border_color", 0xff4c566a)
color("viewer", "mark_color", 0xff808080)
color("viewer", "window_background", 0xff2e3440)
color("text", "shadow", 0x0d000000)
color("text", "background", 0x002e3440)
...
S.imagelist.enable_recursive(false) -- recursive directory reading
...
S.text.set_size(12)          -- font size in pixels
...
for k, dir in pairs { h = "left", l = "right", j = "down", k = "up" } do
  mapg(k, function()
...
mapg("w", function()
  local img = assert(S.gallery.get_image())
...
S.gallery.set_selected_scale(1.15)    -- scale for selected thumbnail
